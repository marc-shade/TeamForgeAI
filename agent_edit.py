...
from agent_interactions import process_agent_interaction, generate_and_display_images # Correct the import here
...
def open_edit_agent(index: int) -> None:
    ...
    # st.rerun() # Removed st.rerun() from here

def assign_skills(role: str) -> list:
    """Assign skills to an agent based on its role."""
    role_to_skills_mapping = {
        "Project Manager": ["generate_agent_instructions", "update_project_status"], # Updated skills
        ...
        "Moral Lesson Consultant": ["generate_agent_instructions", "update_project_status"], # Updated skills
    }
...
def select_model(skills: list) -> str:
    ...
    if skills:
        skill_based_model_mapping = {
            'generate_agent_instructions': 'mistral:7b-instruct-v0.2-q8_0', # Added new skill mapping
            'update_project_status': 'mistral:7b-instruct-v0.2-q8_0', # Added new skill mapping
...
def handle_agent_editing(teams: list, agents_base_dir: str) -> None: # Pass agents_base_dir
    ...
    if show_edit:
        if edit_index is not None and 0 <= edit_index < len(st.session_state.agents_data):
            ...
            with st.expander(f"Edit Properties of {agent['config'].get('name', '')}", expanded=True):
                edit_agent_properties(agent, teams, agents_base_dir) # Pass agents_base_dir


def edit_agent_properties(agent: dict, teams: list, agents_base_dir: str) -> None: # Pass agents_base_dir
    ...
    if st.session_state[f"save_clicked_{edit_index}"]:
        ...
        save_agent_to_json(agent, os.path.join(agents_base_dir, st.session_state["current_team"], f"{new_name}.json"))
        if old_name != new_name:
            os.remove(os.path.join(agents_base_dir, st.session_state["current_team"], f"{old_name}.json"))
        st.session_state["trigger_rerun"] = True
        st.session_state[f"save_clicked_{edit_index}"] = False
    ...
    if st.button("Move Agent", key=f"move_agent_{edit_index}"):
        source_path = os.path.join(agents_base_dir, st.session_state.current_team, f"{agent['config']['name']}.json")
        destination_team = move_to_team
        destination_path = os.path.join(agents_base_dir, destination_team, f"{agent['config']['name']}.json")
...
def regenerate_agent_description(agent: dict) -> str:
    ...
    objectives = "\n".join([f"- {objective['text']}" for objective in current_project.objectives]) if current_project else ""
    deliverables = "\n".join([f"- {deliverable['text']}" for deliverable in current_project.deliverables]) if current_project else ""
...
def download_agent_file(expert_name: str) -> None:
    ...
    agents_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "files", "agents")) # Modified path
